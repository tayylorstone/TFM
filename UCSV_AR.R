...
fit_ucsvar <- function(y, nloop = 1000, burnin = 200, plot_results = TRUE) {
  ...
  ah   <- 10; lh   <- .2^2  * (ah - 1)
  phi <- 0.5; c <- rep(0,T); omega2c <- 0.1^2
  ...
  for (loop in 1:nloop) {
    ...
    # update AR component
    for(t in 2:T){
      c[t] = phi * c[t-1] +rnorm(1,0,sqrt(omega2c))
    }
    ## sample h
    ystar <- log((y - tau - c)^2 + .0001)
